...
def load_dataset(dataset_name, balance=False, discretize=True, dataset_folder='./'):
    ...
    if dataset_name == 'adult':
    ...
    elif dataset_name == 'tenyear':
        feature_names = ["studycode","dos",'FITDate','age','sex','bmi','target_heart_rt','mets_achieved','mets_c','resting_heart_rt',
        				'resting_systolic','resting_diastolic','peak_heart_rt','peak_systolic','peak_diastolic','percent_hr_achieved',
        				'adequatestudy85','maximalstudy','norm_rest_ekg_flg','warfarin','amio','aspirin','bb','acei','arb','diuretic',
        				'ccb','other_htnmed','htnmed','statin','other_lipid','all_lipid','insulin','dmmed','depressionmed','smokingmeds',
        				'plavix','nitrates','edmeds','dig','lungdiseasemeds','thyroidmeds','htn','afib','dm','iddm','hyperlipid',
        				'rheumaticfever','famhx','oldchf','priorCVA','aflutter','smoke','knowncad','lastseen','newdmdate','newdm',
        				't_to_newdm','obesity2','sedentary2','newdm10','FU','FU10','black','risk']
        features_to_use = [3,9,10,11,15,21,42,46,48,58,59,63]
        categorical_features = [21,42,46,48,58,59,63]
        cat_map = {'0': 'No', '1': 'Yes'}

        transformations = {
            58: lambda x: map_array_values(x, cat_map),
            59: lambda x: map_array_values(x, cat_map),
            63: lambda x: map_array_values(x, cat_map),
            60: lambda x: map_array_values(x, cat_map),
        }
        dataset = load_csv_dataset(
            os.path.join(dataset_folder, 'diabetes/Ten year.csv'), 60, ', ',
            feature_names=feature_names, features_to_use=features_to_use,
            categorical_features=categorical_features, discretize=discretize,
            balance=balance, feature_transformations=transformations)
        ...
        features_to_use = [2,4,5,10,13,18,22,28,31,45,46,48,49,64]
        categorical_features = [4,5,22,28,31,45,46,48,49,64]
        cat_map = {'0': 'No', '1': 'Yes'}

        transformations = {
            64: lambda x: map_array_values(x, cat_map),
        }
        dataset = load_csv_dataset(
            os.path.join(dataset_folder, 'mortality/Mortality.csv'), 61, ', ',
            feature_names=feature_names, features_to_use=features_to_use,
            categorical_features=categorical_features, discretize=discretize,
            balance=balance, feature_transformations=transformations)
